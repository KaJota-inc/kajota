  ...
  return (
    <>
      ...
      <View style={styles.subInner}>
        <View>
          <Text style={styles.itemCategory}>{item?.store}</Text>
        </View>
        <View>
      ...
      <View style={styles.cardBottom}>
        <View style={styles.cardBottomSub}>
          <View style={styles.cardBottomPriceBody}>
            <Text style={styles.cardBottomPrice}>
              {item?.ccy}
              {Number(item?.price || 0).toFixed(2)}
            </Text>
          </View>
          <View style={styles.cardBottomSubA}>
            {Array.from({ length: item?.rating || 0 }).map((_, k) => (
              <View key={nanoid()}>
                <AntDesign color={COLORS.light.star} name="star" size={12} />
              </View>
            ))}

            <Text style={styles.cardBottomRating}>{item?.rating}</Text>
